...
<Mermaid chart={`
flowchart TD
  MergePR["Merge PR"] --> BranchCheck{"Branch prefix?"}
  BranchCheck -->|"release/*"| NativePath["Native Build Path"]
  BranchCheck -->|"feat/* bugfix/* chore/*"| OTAPath["OTA Update Path"]
  BranchCheck -->|"hotfix/*"| UrgentOTA["OTA Update Path (urgent)"]
  NativePath --> TargetNative{"Target branch?"}
  OTAPath --> TargetOTA{"Target branch?"}
  UrgentOTA --> TargetOTA
  TargetNative -->|staging| TestFlight["TestFlight / Internal Track"]
  TargetNative -->|main| AppStores["App Store + Google Play"]
  TargetOTA -->|staging| StagingOTA["Staging OTA"]
  TargetOTA -->|main| ProdOTA["Production OTA"]
`} />
...
<Mermaid chart={`
flowchart LR
  Script["release-workflow.sh 3.6.0"] --> Branch["Create release/3.6.0\nfrom main"]
  Branch --> PR["Open PR to\nstaging or main"]
  PR --> Merge["PR merged"]
  Merge --> Extract["Extract version\nfrom branch name"]
  Extract --> Sync["Sync EXPO_PUBLIC_APP_VERSION\nin EAS"]
  Sync --> Build["eas build --auto-submit\niOS + Android"]
  Build --> Tag["Create git tag\nv3.6.0"]
  Tag --> Slack["Slack notification\nvia EAS webhook"]
`} />
...
<Mermaid chart={`
flowchart LR
  Merge["feat/* PR merged"] --> Workflow["GitHub Actions\nota-staging.yml"]
  Workflow --> ReadVer["Read version\nfrom EAS"]
  ReadVer --> Bump["Bump patch\n3.5.4 → 3.5.5"]
  Bump --> Update["eas update\npublish JS bundle"]
  Update --> Notify["PUT /v2/config/ota\nnotify backend"]
  Notify --> Toast["App shows\nrestart toast"]
`} />
...
<Mermaid chart={`
flowchart LR
  NB1["Native Build\nsets 3.6.0"] --> OTA1["OTA #1\n3.6.1"]
  OTA1 --> OTA2["OTA #2\n3.6.2"]
  OTA2 --> NB2["Native Build\nsets 3.7.0"]
  NB2 --> OTA3["OTA #1\n3.7.1"]
  OTA3 --> OTA4["OTA #2\n3.7.2"]
`} />
